...
class McCommand extends Command {
    ...
    async exec(message) {
        ...
        let cur = '';
        cur = message.content.toLowerCase().replace('lca', '').replace('mc', '').replace('marketcap', '').trimStart().trimEnd();
        if (cur === '') {
            message.channel.send('OOPS! add a parameter like \`lca mc ethereum\` or \`lca mc btc\` or a rank number like \`lca mc 7\`')
        }
        console.log(cur)
        if (cur === 'rick') {
